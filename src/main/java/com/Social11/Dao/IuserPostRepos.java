...
//	public List<Post_data> fetchAllPost(@Param("user_id") int user_id); 
	...
	@Query(value = "SELECT * from post_da1 inner join user_post on post_da1.post_id = user_post.post_id where user_post.user_id in (SELECT friend_id from user_fri_map where user_fri_map.user_id =:user_id) OR user_post.user_id =:user_id",
	...
	@Query(value = "SELECT * from post_da1 inner join user_post on post_da1.post_id = user_post.post_id where user_post.user_id =:user_id" ,nativeQuery = true)
	public List<Post_data> fetchMyPost(@Param("user_id") int user_id);		
